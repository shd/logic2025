...
\item Давайте разберёмся во взаимоотношениях различных формулировок закона исключённого третьего и подобных
...
\begin{enumerate}
\item Все учёные занимаются умственным трудом. Некоторые учёные не являются городскими жителями.
...
Нельзя использовать рефлексию, работу с файлами и другие конструкции языка, дающие доступ к 
...
$(y_0, y_1, y_2,\dots) \in r(a)$, то $x_i = y_i$, либо $|x_i - y_i| \in \{1,9\}$.
\end{itemize}
...
\item Существуют ли утверждения, доказательство которых не может иметь порядок, меньший $\omega$?
...
\item Докажите, что любой частичный порядок может быть расширен до линейного (формально: пусть $(X,\leqslant)$ -- частично упорядоченное множество, тогда существует $(\leqslant')\subseteq X^2$ -- линейный порядок такой, что $(\leqslant)\subseteq(\leqslant')$).

\item Пусть $\mathcal F$ -- множество финитных последовательностей натуральных чисел. Введем отношение $(\preceq)\subseteq \mathcal F^2$ -- отношение доминирования ($a\preceq b\Leftrightarrow \forall i\in\mathbb N.\,a_i\leqslant b_i$). Докажите, что $(F,(\preceq))\simeq (\mathbb N,\{(a,b): \exists c:ac=b\})$.
...
\begin{enumerate}
    \item Докажите, что $F(\alpha,\beta)=\{f:\alpha\to \beta\}$ -- множество всех функций из $\alpha$ в $\beta$ с конечным носителем -- может быть вполне упорядочено так, чтобы порядковый тип эквивалентен $\beta^\alpha$ (далее принимайте эту формулировку в качестве определения степени через порядковые типы).
...
\begin{enumerate}
    ...
    \item Пусть $\alpha,\beta$ -- ординалы. Докажите, что ординал $\gamma<\alpha^\beta$ тогда и только тогда, когда существуют $\beta_k<\beta_{k-1}<\dots<\beta_1<\beta$ и $\alpha_1,\dots,\alpha_k<\alpha$ такие, что $\gamma=\alpha^{\beta_1}\cdot\alpha_1+\cdots+\alpha^{\beta_k}\cdot\alpha_k$. Докажите, что такое представление $\gamma$ в виде суммы (называемое Канторовой нормальной формой) единственно.
\end{enumerate}
...
\item Пусть $A,B$ удовлетворяют определению (***) из предыдущего задания. Введем отношение $(\leqslant)$ на $A\times B$ 
...
\item Пусть $(X,\preceq)$ -- полная решетка, заданная на множестве $X$, (решетка называется полной, если для любого $\varnothing\neq Y\subseteq X$ существуют точные верхняя и нижняя границы). Функция $F:X\to X$ -- монотонна (т.е. $\forall x,y\in X.\,x\preceq y\to F(x)\preceq F(y)$). Докажите, что существует наименьшая неподвижная точка $F$, т.е. элемент $x\in X$, что $(F(x)=x)\,\&\,\forall y\in X.\, F(y)=y\to x\preceq y$.
...
работу семафоров.
\item Рассмотрим светофор на перекрёстке двух дорог, без допсекций. Будем считать,
...
\item Постройте недетерминированный обобщённый автомат Бюхи для $\bigcirc\alpha$
