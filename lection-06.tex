...
\begin{enumerate}
\item $D$ --- множество всевозможных предметных выражений без предметных переменных. Оно непусто (язык обычно содержит много нульместных функций), 
...
\begin{lmm}Пусть $\varphi$ --- замкнутая бескванторная формула, тогда $\mathcal{M}\models\varphi$ тогда и только тогда, когда $\varphi\in M$.
\end{lmm}\pause
...
\item Пусть $d^k_i$ --- семейство \emph{свежих} констант, в $M_{i-1}$ не встречающихся.\pause
...
\begin{proof}
\begin{itemize}
\item Пусть это не так, и существует замкнутая формула $\varphi$, что $\models\varphi$, но $\not\vdash\varphi$.\pause
