...
\begin{dfn}Отношение $W_1$ рекурсивно, поэтому выразимо в Ф.А. некоторой формулой $\omega_1$ со свободными переменными $x_1$ и $x_2$, причём:
\begin{enumerate}
...
--- и это можно доказать, то есть $\vdash\text{Consis}\rightarrow\sigma(\overline{\ulcorner\sigma\urcorner})$. \pause Однако
...
\begin{lmm}[об автоссылках]
Для любой формулы $\phi(x_1)$ можно построить 
